      ...
      <Dialog open={csvDialog} onOpenChange={setCsvDialog}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Preview of {csvPreview.length} assignment
              {csvPreview.length !== 1 ? "s" : ""} from "{csvFileName}". Missing
              members, teams, and projects will be created automatically.
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="bg-muted/40 rounded-md p-3 space-y-1.5 text-xs">
                ...
                {autoInfo.newTeams.size > 0 && (
                  <p className="text-muted-foreground">
                    Teams:{" "}
                    {Array.from(autoInfo.newTeams).map((t) => (
                      <Badge
                        key={t}
                        variant="outline"
                        className="text-[10px] mr-1 mb-0.5"
                      >
                        {t}
                      </Badge>
                    ))}
                ...
                {autoInfo.newMembers.size > 0 && (
                  <p className="text-muted-foreground">
                    Members:{" "}
                    {Array.from(autoInfo.newMembers).map((m) => (
                      <Badge
                        key={m}
                        variant="outline"
                        className="text-[10px] mr-1 mb-0.5"
                ...
                {autoInfo.newProjects.size > 0 && (
                  <p className="text-muted-foreground">
                    Projects:{" "}
                    {Array.from(autoInfo.newProjects).map((p) => (
                      <Badge
                        key={p}
                        variant="outline"
                        className="text-[10px] mr-1 mb-0.5"
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y">
                {csvPreview.map((row, i) => {
                  const isNewMember = !members.some(
                    (m) => m.name.toLowerCase() === row.member.toLowerCase(),
                  );
                  const isNewProject = !projects.some(
          ...
          {csvPreview.length === 0 && (
            <p className="text-sm text-muted-foreground text-center py-4">
              No valid rows found. Expected columns: Member, Team, Role,
          ...
          <DialogFooter>
            ...
            >
              Import {csvPreview.length} Assignment
